...
namespace RhCalcApp
{
    ...
    {
        static public async Task GenerateSolution()
        {
            ...
            times = await GetTimesDataAsync(timePath);
            intensity = await getIntensityDataAsync(intPath);
            ...
            if (times.Count != 0 && intensity.Count != 0)
            {
                ...
                //Opening website
                OpenWebsite(@"website\index.html");
            }

            //Function to read data asynchronously from the time.txt file and then convert it to a List<>
            async Task<List<int>> GetTimesDataAsync(string path)
            {
                try
                {
            ...
            //Function to read data asynchronously from int.txt file and the convert it to a Dictionary<><>
            async Task<Dictionary<int, double>> getIntensityDataAsync(string path)
            {
                try
                {
            ...
            void OpenWebsite(string path)
            {
                ...
                {
                    MessageBox.Show($"The file '{path}' was not found.");
                }
            }
        }
    }
